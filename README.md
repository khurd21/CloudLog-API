...
```bash
Logbook-Lambda$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
Logbook-Lambda$ sam local start-api
Logbook-Lambda$ curl http://localhost:3000/
```
...
```bash
Logbook-Lambda$ sam logs -n NetCodeWebAPIServerless --stack-name Logbook-Lambda --tail
```
...
```bash
Logbook-Lambda$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name Logbook-Lambda
```
